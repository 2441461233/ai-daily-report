...
def peak_multiplier(now):
    ...
    return (
        1.0 if now.weekday() < 5 and (1 <= now.hour < 4 or 6 <= now.hour < 10) else 0.5
    )
...
def usage_cost_usd(usage, factor=1.0):
    ...
    if (
        any(
            ...
            for x in (prompt, completion, cached)
        )
        or cached > prompt
    ):
        raise shared.QwenReportError("DeepSeek returned invalid token usage")
    return (
        ((prompt - cached) * 0.3 + cached * 0.006 + completion * 1.2)
        * factor
        / 1_000_000
    )
...
def scalar_numbers(text):
    text = text.replace("一半", "0.5").replace("个百分点", "%").replace("百分点", "%")
    values = set()
    for value in shared.normalized_numbers(text):
        number = re.match(r"[+-]?\d+(?:\.\d+)?", value)
        if number:
            values.add(number.group())
    for word, number in (
        ("one", "1"),
        ("two", "2"),
        ("three", "3"),
...
def validate_translated_claim(headline, summary, evidence):
    ...
    if extra:
        raise shared.QwenReportError(
            f"{headline}: unsupported numeric values {sorted(extra)}; omit inferred counts"
        )
    ...
    unknown = [
        x
        for x in shared.unsupported_claim_markers(fact, evidence)
        if re.search(r"[A-Z0-9_.]", x) and x not in {"RSS", "HN", "Trending"}
    ]
    if unknown:
        raise shared.QwenReportError(f"{headline}: unknown identifiers {unknown}")
    problem = semantic_grounding_error(fact, evidence)
    if problem:
        raise shared.QwenReportError(f"{headline}: {problem}")


def validate_draft(document, cards, mode):
    """Return all item errors together so a repair fixes the entire draft."""
    problems = []

    def check(headline, summary, evidence):
        try:
            validate_translated_claim(headline, summary, evidence)
        except shared.QwenReportError as exc:
            problems.append(str(exc))

    shared.compile_sections(document, cards, mode, claim_validator=check)
    if problems:
        raise shared.QwenReportError("Draft validation errors:\n" + "\n".join(problems))


class Client:
    def __init__(self, key, diagnostics, cap_usd=0.45):
        if not key:
            raise shared.QwenReportError("DEEPSEEK_API_KEY is not configured")
        if not math.isfinite(cap_usd) or cap_usd <= 0:
            raise shared.QwenReportError("cost cap must be positive")
        self.key = key
    ...
    @property
    def spent(self):
        return sum(c["estimatedCostUsd"] for c in self.calls)
        ...
        shared.write_diagnostics(
            self.path,
            {
                **self.context,
                ...
                "costCapUsd": self.cap,
                "estimatedCostUsd": round(self.spent, 8),
                "calls": self.calls,
            },
        )
    ...
    def preflight(self):
        ...
        if MODEL not in {m.get("id") for m in models.get("data", [])}:
            raise shared.QwenReportError(
                "DeepSeek V4.1 Flash is not available to this API key"
            )
    ...
    def _call_once(self, stage, system, user, schema, max_tokens=12000):
        ...
        reservation = (
            len((system + user).encode()) * 0.3 + max_tokens * 1.2
        ) / 1_000_000
        if self.spent + reservation > self.cap:
            raise shared.QwenReportError(
                "DeepSeek worst-case reservation exceeds run cost cap"
            )
        ...
        self.calls.append(
            {
                ...
                "pricingMultiplier": peak_multiplier(now),
                "startedAt": now.isoformat(),
                "finishReason": (response.get("choices") or [{}])[0].get(
                    "finish_reason"
                ),
            }
        )
        ...
        if not shared.response_matches_model(response.get("model"), MODEL):
            raise shared.QwenReportError(
                "DeepSeek response model does not match deepseek-flash"
            )
...
def source_cards(selection, sources, priorities, exclusions):
    ...
    for item in selection.get("selections", []):
        ...
        if (
            ...
            or source_id in used
            or section not in shared.SECTION_TITLES
        ):
            raise shared.QwenReportError(
                "research selected an unknown, duplicate, or invalid source"
            )
        source = by_id[source_id]
        # The collector's fixed section is authoritative (e.g. arXiv/Trending).
        section = source.get("section") or section
        if (
            section == shared.SECTION_TITLES[5]
            and source["dateBasis"] != "trending-observation"
        ):
            raise shared.QwenReportError(
                "GitHub section requires observed daily Trending membership"
            )
        if (
            source["dateBasis"] == "community-discussion"
            and section == shared.SECTION_TITLES[0]
        ):
            raise shared.QwenReportError(
                "undated community discussion cannot establish a new release"
            )
        if shared.matches_attachment_event(
            source["title"], source["text"], [x["title"] for x in exclusions]
        ):
            continue
        used.add(source_id)
        ...
        if source["publisher"].startswith("arXiv") and source["dateBasis"].startswith(
            "publisher-feed"
        ):
            date_text += "For arXiv RSS this is the announcement date, not the submission or revision date; preserve the original paper's dates when discussing them. "
        text = (
            "Publisher: "
            + source["publisher"]
            + "\n"
        ...
        if source.get("discussionUrl"):
            text += " Hacker News discussion."
        urls = [{"name": source["publisher"], "url": source["url"]}]
        if source.get("discussionUrl"):
            urls.append({"name": "Hacker News 讨论", "url": source["discussionUrl"]})
        cards.append(
            {
                "id": source_id,
                "section": section,
    ...
    for title, minimum, _ in shared.SECTION_POLICY:
        ...
        if count < minimum:
            raise shared.QwenReportError(
                f"{title}: only {count} usable sources; requires {minimum}"
            )
    if shared.main_report_capacity(cards) < 20:
        raise shared.QwenReportError(
            "not enough independent evidence for 20 report items"
        )
    return cards
...
def select_sources(client, document, priorities, exclusions, history=()):
    ...
    user = (
        "请为完整日报选出 26–38 个独立选题（来源不足时如实返回）。六板块和目标条数："
        + json.dumps(shared.SECTION_POLICY, ensure_ascii=False)
        + """。
...
已强制入选的优先候选："""
            ...
            "selections": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "section": {
                            "type": "string",
                            "enum": list(shared.SECTION_TITLES),
                        },
                    },
    ...
    schema["properties"]["selections"]["items"]["properties"]["sourceId"]["enum"] = [
        s["id"] for s in document["sources"]
    ]
    correction = ""
    for attempt in range(1, 3):
        selection = client.call(
            f"research-selection-{attempt}", system, user + correction, schema, 10000
        )
        if client.path:
            shared.write_diagnostics(
                client.path.parent / f"deepseek-selection-{attempt}.json", selection
            )
        try:
            ...
            if attempt == 2:
                raise
            correction = (
                "\n请修正上一轮选题后输出完整 selections。采集器固定 section 的原文按其固定板块入选。问题："
                + str(exc)
                + "\n上一轮："
...
def run(arguments):
    client = Client(
        ...
        float(os.environ.get("DEEPSEEK_COST_CAP_USD", "0.45")),
    )
    ...
    try:
        ...
        if now is None:
            raise shared.QwenReportError(
                "generated-at must be a timezone-aware timestamp"
            )
        priority = shared.load_json(arguments.priority)
        builders = shared.load_json(arguments.builders)
        exclusions = shared.attachment_exclusions(
            shared.load_json(arguments.artificial_analysis),
            shared.load_json(arguments.waytoagi),
        )
        priorities = shared.priority_cards(
            priority, shared.fallback.existing_priority_ids(arguments.artifact_dir)
        )
        mode = shared.artifact_mode(arguments.artifact_dir, arguments.date)
        diag_dir = (
            arguments.diagnostics.parent
            if arguments.diagnostics
            else Path("/tmp/deepseek-diagnostics")
        )
        diag_dir.mkdir(parents=True, exist_ok=True)
        if mode == "addendum":
            if not priorities:
                raise shared.QwenReportError(
                    "no uncovered priority candidate needs an addendum"
                )
            cards = priorities
        else:
            print(
                "deepseek report: collecting dated public source evidence", flush=True
            )
            frozen = os.environ.get("DEEPSEEK_EVIDENCE_FILE")
            document = (
                shared.load_json(Path(frozen))
                if frozen
                else collector.collect(now, builders, arguments.artifact_dir)
            )
            shared.write_diagnostics(diag_dir / "deepseek-sources.json", document)
            client.save(
                sourceCount=len(document["sources"]), sourceErrors=document["errors"]
            )
            print(
                f"deepseek report: selecting from {len(document['sources'])} source records",
                flush=True,
            )
            cards = select_sources(
                client,
                document,
                priorities,
                exclusions,
                shared.reported_history(arguments.reported, now.replace(tzinfo=None)),
            )
        shared.write_diagnostics(diag_dir / "deepseek-evidence.json", {"cards": cards})
        system, user = shared.editor_prompt(
            arguments.date, [{**card, "extractorOutputs": []} for card in cards], mode
        )
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                shared.validate_factual_audit(
                    audit, draft, cards, grounding_checker=semantic_grounding_error
                )
                output = shared.build_artifact(
                    arguments,
                    draft,
                    cards,
                    mode,
                    claim_validator=validate_translated_claim,
                )
                client.save(
                    status="success",
                    artifact=output.name,
                    artifactSha256=hashlib.sha256(output.read_bytes()).hexdigest(),
                    evidenceCount=len(cards),
                    itemCount=sum(len(s["items"]) for s in draft["sections"]),
                )
                print(
                    f"deepseek report: wrote {output.name}; estimated USD {client.spent:.6f}",
                    flush=True,
                )
                return output
            except shared.QwenReportError as exc:
        ...
        client.save(
            status="failed", error=str(exc)[:1600], errorType=type(exc).__name__
        )
        raise
