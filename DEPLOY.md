...
## 1. GitHub Actions 与 Copilot 权限

日报工作流不再读取 Kimi/Moonshot 或其他模型 API Key。研究 Agent 使用 GitHub Actions 自带的短期
`GITHUB_TOKEN` 调用 GitHub Copilot CLI；`generate` job 已声明 `copilot-requests: write`，不需要创建 PAT
或 Repository Secret。

- 个人仓库：仓库所有者的 GitHub 账号需要能使用 Copilot CLI；请求计入该账号的 Copilot 方案。
- 组织仓库：还需在组织 Copilot policy 中允许 Actions 使用 Copilot CLI 并由组织计费。
- Copilot 无权限、额度暂时不可用、安装失败或输出未通过门禁时，工作流会自动改用已验证的官方候选与
  builder feed，发布带 `·自动恢复版` 标签的确定性日报；因此模型通道故障不会再造成整日空档。

Copilot 在 `$RUNNER_TEMP` 的仓库副本中运行，禁止 Shell；只有通过内容与官方候选覆盖门禁的新日报文件
才会被复制回主工作区。半成品和越界修改不会进入提交。
...
来源降级记录、Copilot session/recovery log、质量门输出与 git diff，便于区分「未发现」、「核验失败」和「覆盖失败」。
...
- 若当天无 commit，优先看 Daily AI report 日志；质量门会明确报告 Copilot/恢复版状态、来源失败、JSON/URL
